...
const LOGOS = [CoinMetricsLogo, A16Z, BEN, BAF, ENCODE];

const LogoCloud = () => {
  return (
    <div className="grid  md:grid-cols-2 gap-10 sm:grid-cols-1 lg:grid-cols-3">
      {LOGOS.map((logo) => (
        <div className="w-60 flex items-center justify-center p-6 col-span-1">
          <img src={logo}></img>
        </div>
      ))}
    </div>
